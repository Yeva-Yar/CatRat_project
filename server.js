...
createServer(function (req, res) {
    try {
        if (req.method === 'POST' && req.url === '/api/register') {
            ...
            if (!user) {
                let page = fs.readFileSync(path.join(__dirname, "static", "pages", "auth", "auth.html"), "utf-8");
...
function register(req, res) {
    ...
    let body = '';
            ...
            req.on('end', async () => {
                ...
                const sql = 'INSERT INTO Users (login, password, name, surname) VALUES (?, ?, ?, ?)';
                db.query(sql, [login, hashedPassword, name, surname], (err) => {
                    if (err) {
                        ...
                        res.end(JSON.stringify({ message: 'Користувач зареєстрований' }));
                    }
                });
            })
}
